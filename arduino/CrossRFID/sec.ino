...
struct Motor {
  ...
  int speed_delta;
  Motor() {}
  Motor(int pwm_pin, int dir_pin1, int dir_pin2, int speed_delta = 20)
    : pwm_pin(pwm_pin), dir_pin1(dir_pin1), dir_pin2(dir_pin2), speed_delta(speed_delta), now_dir(-1), now_speed(0) {
...
struct ForwardState : StateSequenceNode {
  ...
  ForwardState(bool isPostTurn)
    : duration(isPostTurn ? 450UL : 630UL), startTime(0), timingStarted(false) {
    state = FORWARD;
  }
...
struct TurnRightState : StateSequenceNode {
  ...
  const unsigned long duration = 1400;
...
public:
  ...
  void turnRight(int speed) {
    ...
    right_motor.setSpeed(0.2 * speed * motor_speed_bias);
  }
  void turnLeft(int speed) {
    left_motor.setSpeed(0.2 * speed);
