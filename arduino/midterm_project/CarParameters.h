...
int turn_speed = 100;
double turn_speed_ratio = 0.3; // fast wheel speed divided by slow wheel speed
// tracking
...
bool forwardExtraEndCondition(int ir_result[7], int left_speed, int right_speed) {
    return true; //(ir_result[2] && ir_result[3] && ir_result[4] && ir_result[1] && ir_result[5]);
}
bool turnLeftExtraEndCondition(int ir_result[7], int left_speed, int right_speed) {
    return (ir_result[2] || ir_result[3] || ir_result[4]) && !ir_result[0] && !ir_result[6];//ir_result[0] || ir_result[1] || ir_result[2] || ir_result[3];
}
bool turnRightExtraEndCondition(int ir_result[7], int left_speed, int right_speed) {
    return (ir_result[2] || ir_result[3] || ir_result[4]) && !ir_result[0] && !ir_result[6];//ir_result[3] || ir_result[4] || ir_result[5] || ir_result[6];
}
