...
ull turn_left_duration = 450;
ull turn_right_duration = 500;
...
bool turnLeftExtraEndCondition(int ir_result[7], int left_speed, int right_speed) {
    return ir_result[2]; //ir_result[0] || ir_result[1] || ir_result[2] || ir_result[3];
}
bool turnRightExtraEndCondition(int ir_result[7], int left_speed, int right_speed) {
    return ir_result[4]; //ir_result[3] || ir_result[4] || ir_result[5] || ir_result[6];
}
