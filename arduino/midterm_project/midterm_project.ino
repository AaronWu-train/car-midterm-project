...
public:
    ...
    void update() {
        ...
        // IR
        for (int i = 0; i < 7; ++i) ir_result[i] = digital_ir[i].read();
        // send idle if state queue is empty
        if (state_queue.empty()) {
            if (!idle_signal_sent) bluetooth_transmitter.sendCarIsNowIdle(), idle_signal_sent = true;
            if (bluetooth_transmitter.checkRemoteCommandStreamInput(state_queue)) idle_signal_sent = false, current_state_start_time = millis();
            return;
        }
        // state transistions
        ...
        if (current_state.state == State::PossibleState::FORWARD) {
            int sum = 0;
            double weight_sum = 0;
            for (int i = 0; i < 7; ++i) sum += ir_result[i], weight_sum += ir_weight[i] * ir_result[i];
            ...
            left_motor.setSpeed((forward_speed + correction));
            right_motor.setSpeed((forward_speed - correction) * motor_speed_bias);
            ...
            right_motor.setSpeed(turn_speed_ratio * turn_speed * motor_speed_bias);
